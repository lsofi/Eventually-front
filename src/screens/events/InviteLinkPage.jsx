...
export default function InviteLinkPage() {
    ...
    const getEvent = async () => {
        //console.log('Valor del hash recuperado', hash)
        setModalLoading(prev=>prev+1); // Acivates the loading modal
        try{
            const resEventId = await axios.get(`../api/event/getEventByHash?hash=${hash}`);
            setEventId(resEventId.data);
            const res = await axios.get(`../api/event/getEventInfo?event_id=${resEventId.data}`);
            const resEvent = res.data;
            setEvent(resEvent);
            getLocationNames(resEvent.address);
        } catch (error){
            setSuccessMessage('Este link de invitación no es válido, ya que ha alcanzado la fecha límite de validez. Solicita otro enlace.')
    ...
    const handleRespondInvitation = async (accepted) => {
        ...
        try{
            const respuesta = await axios.put('../api/guest/respondInvitation', params);
            if(respuesta){
                const acceptedMsg = accepted? 'aceptada': 'rechazada';
                if(accepted) setNavigateUrl(`/events/event?event_id=${eventId}`);
    ...
    return (
        <div className="body d-flex flex-column invite">
            <div style={{backgroundColor: "var(--card)"}}>
                <img src="/resources/images/EventuallySmallLogoLight.png" style={{height: "5vh"}} className="m-2"/>
            </div>
            ...
            <div style={{backgroundColor: "var(--card)"}} className="d-flex flex-column align-items-center mt-5">
                <img src="/resources/images/EventuallySmallLogoLight.png" style={{height: "5vh"}} className="m-1"/>
                <p>@{year} Argentina</p>
            </div>
