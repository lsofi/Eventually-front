...
import { faCheck, faPlusCircle, faFile } from '@fortawesome/free-solid-svg-icons';
...
export default function PollConfig( props ) {
    ...
    const getPoll = async () => {
        ...
        try{
            const res = await axios.get(`../api/poll/getPoll?poll_id=${poll_id}&event_id=${event_id}`);
            const resPoll = res.data;
            setPoll({
                has_answers:resPoll.has_answers,
                name: resPoll.name,
                visible: resPoll.visible
            });
            const questions = resPoll.questions.map((question) => ({
                question_id: question.question_id,
                question: question.question,
                possible_answers: question.possible_answers,
                type: question.type,
                max_length: question.max_length,
                mark0: question.mark0,
                mark100: question.mark100
            }))
    ...
    const findErrors = () => {
        ...
        pollQuestions.forEach(question => {
            newErrors = {...newErrors, ...findErrorsQuestion(question, true)};
        })

        return newErrors;
    }

    const findErrorsQuestion = (question, quiet) => {
        let newErrors = {}
    ...
    const handleToggleVisible = () => {
        setPoll(prev => ({...prev, visible: !prev.visible}));
    }
    ...
    const handleOpenPollReview = () => {
        navigate(`/events/reviewPoll?event_id=${event_id}&poll_id=${poll_id}`)
    }
        ...
        if ( Object.keys(newErrors).length > 0 ) {
          // We got errors!
            ...
            try {
                setLoading(prev=>prev+1);
                setErrors({});
                await axios.put('../api/poll/updatePoll', params);
                setLoading(prev=>prev-1)
                setInfoModal({show: true, message: '¡Encuesta modificada con éxito!', img: SuccessPhoneGirl});
            } catch (error) {
                console.log(error)
                setLoading(prev=>prev-1);
            }
    ...
    const handleOnRespond = async () => {
        ...
        const params = {
            event_id: event_id,
            poll_id: poll_id,
            answers: pollQuestions.map((question)=> ({
                question_id: question.question_id,
        ...
        try {
            setLoading(prev=>prev+1);
            setErrors({});
            await axios.post('../api/poll/respondPoll', params);
            setLoading(prev=>prev-1)
            setInfoModal({show: true, message: '¡Respuesta enviada con éxito!', img: SuccessPhoneGirl});
        } catch (error) {
            console.log(error)
            setLoading(prev=>prev-1);
        }
    ...
    return (
        <div className="body d-flex flex-column nav-bar-content padding-navbar">
            ...
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
                {props.modify?
                <>
                    <div className="bg-card py-2 px-3" style={{width: "35rem", borderRadius: '1rem'}}>
                    ...
                    <div className="bg-card py-2 px-3" style={{borderRadius: '1rem'}}>
                        <Form onSubmit={onSubmit}>
                            <Form.Group className="d-flex gap-3" controlId="visible" style={{fontSize: '1.5rem'}}>
                                <Form.Label className="mb-0">Encuesta visible</Form.Label>
                                <Form.Check type="switch" checked={poll.visible} onChange={handleToggleVisible}/>
                            </Form.Group>
                        </Form>
                    </div>
                </>
                ...
                <div className="d-flex gap-4 flex-wrap align-items-center">
                    <h2>{poll.name}</h2>
                    {poll.has_answers && !props.answer && !props.modify? 
                        <Button className="btn btn-primary-body btn-add px-3 py-1" title="Ver reporte" onClick={handleOpenPollReview}>
                            <FontAwesomeIcon icon={faFile}/> Ver reporte
                        </Button>: null
                    }
                </div>}
            </div>
            <div className="mt-3">
                    ...
                    {pollQuestions && pollQuestions.length? 
                        pollQuestions.map((question, index)=> (
                            <PollQuestion 
                                ...
                                findErrorsQuestion={findErrorsQuestion}
                            />
                        ))
                        : null
                    }
                ...
                :
                    props.answer?
                    <OverlayTrigger key={'submit'} placement={'left'} overlay={<Tooltip id={'tooltip-submit'}>Enviar respuesta</Tooltip>}>
                        <Button className="event-submit-button" onClick={handleOnRespond}><FontAwesomeIcon icon={faCheck}/></Button>
                    </OverlayTrigger>
                    : 
                    null
                }
